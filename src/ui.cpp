...
// Forward declarations для функций из main.cpp
...
void ui_loop() {
    ...
    // Авто-закрытие сообщения об ошибке
    ...
    // Рендер при изменении состояния или нажатии
...
void handleOKPress() {
    switch (currentState) {
        ...
        case UI_MAIN_MENU:
            if (nav.index == 0) { currentState = UI_MODULE_SELECT; nav.index = 0; }            if (nav.index == 1) { startModule("WIFI_DEAUTHER"); currentState = UI_MODULE_RUN; }
...
// ======== ОТРИСОВКА ЭКРАНОВ ========void ui_render() {
    tft.fillScreen(TFT_BLACK);
...
void draw_module_select() {
    ...
    for (int i = 0; i < count; i++) {
        uint16_t color = (i == nav.index) ? TFT_YELLOW : TFT_WHITE;        String status = (i == 0 && mods.cc) || (i == 1 && mods.nrf) ? " [OK]" : " [OFF]";
...
void draw_module_run() {
    ...
    if (nav.selectedModule == "NRF24") {
        ...
        drawText("MAC: RANDOM", 15, 70, TFT_GREEN);
    }
