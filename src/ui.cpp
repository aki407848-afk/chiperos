...
// ======== FORWARD DECLARATIONS (ОБЯЗАТЕЛЬНО ДЛЯ C++) ========
void draw_boot();
void draw_main_menu();
...
void ui_init() {
    ...
    tft.init(); tft.setRotation(1); tft.fillScreen(TFT_BLACK); tft.setTextFont(2);
}
    ...
    switch (currentState) {
        ...
        case UI_SETTINGS:
            if (nav.index == 3) {
                ...
                drawText("Reset OK", 20, 100, TFT_GREEN);                delay(800);
            }
            break;
    }
}
...
void draw_boot() {
    ...
    drawText(String(OS_VERSION), 45, 34, TFT_WHITE, 1);    drawText("INIT...", 45, 220, TFT_YELLOW);
}
...
void draw_sdk() {
    ...
    for (int i = 0; i < 4; i++) {
        if (i == nav.index) tft.fillRect(2, 25 + (i * 20), 131, 18, 0x001F);        drawText(files[i], 10, 27 + (i * 20), (i == nav.index) ? TFT_CYAN : TFT_WHITE);
    }
}
...
void draw_settings() {
    ...
    if (nav.index == 2) {
        drawText(String(OS_NAME) + " " + String(OS_VERSION), 10, 130, TFT_WHITE);
        drawText(String(BUILD_DATE) + " " + String(BUILD_CITY), 10, 145, TFT_WHITE);
    }
}
