...
// Активные пины (после детекции храним ТОЛЬКО номер пина, не PinPair!)
...
void detectModules() {
    ...
    // IR    if (tryPin(Pins::IR_TX.primary) && tryPin(Pins::IR_RX.primary, false)) {
        mods.ir = true;
        activeIR_TX = Pins::IR_TX.primary; activeIR_RX = Pins::IR_RX.primary;
    ...
    if (pingSPI(Pins::NRF_CSN.primary)) {
        mods.nrf = true;
        activeNRF_CE = Pins::NRF_CE.primary; activeNRF_CSN = Pins::NRF_CSN.primary;
    ...
    if (pingSPI(Pins::CC_CS.primary)) {
        mods.cc = true;
        activeCC_CS = Pins::CC_CS.primary; activeCC_GDO0 = Pins::CC_GDO0.primary;
    ...
    if (SD.begin(Pins::SD_CS.primary, SPI, 4000000)) {
        mods.sd = true; activeSD_CS = Pins::SD_CS.primary;
...
String getActivePinsString() {
    ...
    s += "NRF:"; s += mods.nrf ? "OK " : "X ";    s += "CC:"; s += mods.cc ? "OK " : "X ";
    s += "SD:"; s += mods.sd ? "OK" : "X";
    return s;
}

// ======== ЗАГЛУШКИ МОДУЛЕЙ ========
// (Оставь как было в Части 2)
