...
// ⚠️ ГЛАВНАЯ ФУНКЦИЯ - ВСЁ ВНУТРИ НЕЁ!
void detectModules() {
    ...
    if (tryPin(Pins::IR_TX.primary) && tryPin(Pins::IR_RX.primary, false)) {
        ...
        mods.ir = true;        activeIR_TX = Pins::IR_TX.backup;
    ...
    if (SD.begin(Pins::SD_CS.primary, SPI, 4000000)) {
        ...
        if (SD.begin(Pins::SD_CS.backup, SPI, 4000000)) {
            mods.sd = true;
            activeSD_CS = Pins::SD_CS.backup;            Serial.println("[HAL] SD: BACKUP");
        } else {
...
void wifi_init() { Serial.println("[WIFI] Init"); }
