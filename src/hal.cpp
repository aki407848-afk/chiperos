...
// Активные пины (храним номера, не структуры)
...
void detectModules() {
    ...
    // ===== IR Module =====    if (tryPin(Pins::IR_TX.primary) && tryPin(Pins::IR_RX.primary, false)) {
        mods.ir = true;
        activeIR_TX = Pins::IR_TX.primary;
        activeIR_RX = Pins::IR_RX.primary;
    ...
    if (SD.begin(Pins::SD_CS.primary, SPI, 4000000)) {
        ...
        pinMode(Pins::SD_CS.backup, OUTPUT);
        digitalWrite(Pins::SD_CS.backup, HIGH);
...
String fs_list_dir(const String& path) {
    ...
    if (!root) root = LittleFS.open(path);    if (!root) return "ERR: PATH";
