...
void setupWebServer() {
    ...
    // API статус (для будущего расширения)
...
// ======== ЗАПУСК МОДУЛЯ (ВЫЗЫВАЕТСЯ ИЗ UI) ========
void startModule(String modName) {
    ...
    // Здесь можно инициализировать конкретное железо модуля
...
void runActiveModule() {
    ...
    // Пример логики: NRF24 Jammer (непрерывный цикл с feedWatchdog)
    if (runningMod == "NRF24") {
        nrf_full_jammer_loop();
    }
    // Пример логики: CC1101 Sniff (пакетный режим)
    else if (runningMod == "CC1101") {
        cc_sniff_loop();
    }

    // Проверка времени выполнения (авто-стоп через 10 мин для безопасности)
...
void loop() {
    ...
    // Если UI вернул состояние в меню из модуля → стоп модуль
