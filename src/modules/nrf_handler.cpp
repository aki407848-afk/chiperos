...
void nrf_full_jammer_loop() {
    ...
    // Быстрая смена каналов для создания интерференции в 2.4GHz ISM
    radio.setChannel(jamChannel);
    radio.openWritingPipe(0xFFFFFFFFFF);
    radio.startWriting();
    
    uint8_t payload[32];
    memset(payload, 0xAA, 32); // Максимальная спектральная плотность
    radio.writeFast(payload, 32);
    
    jamChannel = (jamChannel % 125) + 1; // Hop 1..125
...
void nrf_radio_listen() {
    nrf_init();
    radio.setChannel(76); // Стандартный BLE/WiFi канал
    radio.startListening();
    Serial.println("[NRF] Listening on Ch 76...");
    
    unsigned long start = millis();
    while (millis() - start < 5000) { // Слушаем 5 сек
        if (radio.available()) {
            uint8_t buf[32];
            radio.read(buf, 32);
            Serial.print("[NRF] RX: ");
